...
impl DriveEnumerator for LinuxDriveEnumerator {
    fn enumerate(&self) -> Result<Vec<DriveInfo>> {
        ...
        let entries = fs::read_dir("/sys/block").map_err(|e| DriveWipeError::Io {
        ...
        for entry in entries {
            ...
            match build_drive_info(&name_str, &dev_path) {
...
fn build_drive_info(name: &str, dev_path: &Path) -> Result<DriveInfo> {
    ...
    let firmware_rev = read_sysfs_string(&sys_block.join("device/rev"));

    // Size is reported in 512-byte sectors.
    let size_sectors = read_sysfs_u64(&sys_block.join("size")).unwrap_or(0);
    let capacity = size_sectors * 512;

    // Logical block size.
    let block_size =
        read_sysfs_u64(&sys_block.join("queue/logical_block_size")).unwrap_or(512) as u32;

    // Physical block size.
    let physical_block_size =
        read_sysfs_u64(&sys_block.join("queue/physical_block_size")).map(|v| v as u32);

    // rotational: 0 = SSD/NVMe, 1 = HDD.
    let rotational = read_sysfs_u64(&sys_block.join("queue/rotational"));
    ...
    let transport = detect_transport(name, &sys_block);

    // Removable flag.
    let is_removable = read_sysfs_u64(&sys_block.join("removable")).is_some_and(|v| v == 1);
    ...
    let partition_count = count_partitions(name);
...
fn detect_transport(name: &str, sys_block: &Path) -> Transport {
    if name.starts_with("nvme") {
    ...
    if let Ok(target) = fs::read_link(&subsystem) {
        ...
        if target_str.contains("scsi") || target_str.contains("ata") {
            ...
            let transport_str = read_sysfs_string(&transport_file);
...
/// `/sys/block/<name>/`.
fn count_partitions(name: &str) -> u32 {
    let sys_block = PathBuf::from(format!("/sys/block/{name}"));
